...
def log_message(message: str, level: str = 'INFO', console: bool = True):
    ...
    log_switcher = {'TRACE': logger.trace, 'INFO': logger.info,
...
#     """
#     # Key name that I want to refer on bitwarden, value: actual title
#     # name code, bitwarden
#     items = {
#         "Mundialitis": "Mundialitis Login"
#     }
...
def act_on_element(path: str, action: str, time_range: int = 5):
    # Fluent wait
    """Function that waits a predefined time for an element and acts on it 
    as soon as it finds it.

    Args:
        path (str): [description]
        action (str): [Action to be executed]
        time_range (int, optional): [Wait time]. Defaults to 5.
    """
    ...
    while timer > datetime.now():
        try:
            if action == "click_element":
                # If the element isnt available to click, it will raise an Exception
                ...
                # If the element isnt available to be found, it will raise an Exception
                result = browser.find_element(path)
                return result
        except Exception as e:
            # Fluent wait waits 1 second
            time.sleep(1)
...
def check_file_download_complete(file_extension: str, time_range: int = 10, folder: str = OUTPUT_FOLDER):
    actual_downloaded_files_amount = len(file_system.find_files(
        F'{folder}/*.{file_extension}'))
    ...
    while not downloaded_files_amount == actual_downloaded_files_amount + 1 and timer > datetime.now():
        time.sleep(1)
        downloaded_files = file_system.find_files(
            F'{folder}/*.{file_extension}')
